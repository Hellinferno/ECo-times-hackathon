...
class _TTLCache:
    ...
    def __init__(self, default_ttl: int = 300):
        self._store: dict[str, tuple[float, int, Any]] = {}  # key → (ts, ttl, data)
...
# Macro/structural data changes slowly → 30 min TTL.
# Market-sensitive data (Fear & Greed, quotes) → 5 min TTL.
_ENDPOINT_TTL: dict[str, int] = {
    "/api/economic/v1/getFredSeries": 1800,
    "/api/economic/v1/getFredSeriesBatch": 1800,
    "/api/macro/v1/getMacroSignals": 1800,
    "/api/risk/v1/getRiskScores": 1800,
    "/api/geo/v1/getChokepointStatus": 900,
    "/api/geo/v1/getConflictEvents": 900,
    "/api/market/v1/getMarketImplications": 900,
    "/api/market/v1/getSectorSummary": 900,
    "/api/market/v1/getFearGreedIndex": 300,
    "/api/market/v1/getMarketQuotes": 300,
    "/api/market/v1/getCommodityQuotes": 300,
}
...
class WorldMonitorClient:
    ...
    async def _get(self, path: str, params: dict | None = None) -> dict | None:
        ...
        try:
            ...
            data = resp.json()
            endpoint_ttl = _ENDPOINT_TTL.get(path)
            self._cache.set(cache_key, data, ttl=endpoint_ttl)
            return data
        except (httpx.RequestError, httpx.HTTPStatusError) as exc:
            logger.debug("WorldMonitor request failed: %s %s — %s", path, params, exc)
            return None
        except (ValueError, KeyError) as exc:
            logger.debug("WorldMonitor response parse error: %s %s — %s", path, params, exc)
            return None
    ...
    async def _post(self, path: str, body: dict) -> dict | None:
        ...
        try:
            ...
            logger.debug("WorldMonitor POST parse error: %s — %s", path, exc)
            return None
    ...
    def run_parallel_sync(self, *coros) -> tuple:
        """
        Run multiple coroutines concurrently and return results as a tuple.

        Replaces sequential _run_async calls in sync agent code:
            # Before (sequential — each call blocks):
            a = wm_client.get_macro_signals_sync()
            b = wm_client.get_fear_greed_index_sync()

            # After (parallel — all fire at once):
            a, b = wm_client.run_parallel_sync(
        ...
        # Replace any exceptions with None to preserve graceful-degradation contract
