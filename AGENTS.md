...
This file is the repo-root entry point for Codex and other coding agents working in this monorepo.
...
3. Inside `AI Investment Banking Analyst Agent (AIBAA)/`, the nested `.Codex/` is authoritative for project-specific workflows.
4. `Codex.local.md` is optional personal guidance and must stay uncommitted.
...
- For AIBAA, start with [`AI Investment Banking Analyst Agent (AIBAA)/README.md`](<AI Investment Banking Analyst Agent (AIBAA)/README.md>), then use its nested `.Codex/`.

## Workspace Layout

- `.Codex/commands/`: shared slash-command playbooks.
- `.Codex/rules/`: monorepo-wide guardrails.
- `.Codex/skills/`: reusable workflows for review, deployment, and future optimization work.
- `.Codex/agents/`: reviewer and auditor personas for parallel review loops.
