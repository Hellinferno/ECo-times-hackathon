...
const ALLOWED_ENV_KEYS = new Set([
  'GEMINI_API_KEY', 'GROQ_API_KEY', 'OPENROUTER_API_KEY', 'EXA_API_KEYS', 'BRAVE_API_KEYS', 'SERPAPI_API_KEYS', 'FRED_API_KEY', 'EIA_API_KEY',
...
async function validateSecretAgainstProvider(key, rawValue, context = {}) {
  ...
  try {
    ...
    case 'GEMINI_API_KEY': {
      ...
      if (isCloudflareChallenge403(response, text)) return ok('Gemini key stored (Cloudflare blocked verification)');
      ...
      if (!Array.isArray(payload?.models)) return fail('Unexpected Gemini response');
...
async function dispatch(requestUrl, req, routes, context) {
  ...
  if (requestUrl.pathname === '/api/llm-health') {
    ...
    if (geminiKey) {
      providerChecks.push(
        probeOrigin('https://generativelanguage.googleapis.com').then((available) => ({ name: 'gemini', url: 'https://generativelanguage.googleapis.com', available })),
      );
    }
    ...
    async start() {
      ...
      (async () => {
        const urls = [
          ...
          process.env.GEMINI_API_KEY ? 'https://generativelanguage.googleapis.com' : null,
