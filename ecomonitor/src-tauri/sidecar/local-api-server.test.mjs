...
test('accepts GEMINI_API_KEY via /api/local-env-update', async () => {
  const localApi = await setupApiDir({});
  ...
  try {
    const response = await fetch(`http://127.0.0.1:${port}/api/local-env-update`, {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({ key: 'GEMINI_API_KEY', value: 'gemini-test-key' }),
    });
    assert.equal(response.status, 200);
    const body = await response.json();
    assert.equal(body.ok, true);
    assert.equal(body.key, 'GEMINI_API_KEY');
    assert.equal(process.env.GEMINI_API_KEY, 'gemini-test-key');
  } finally {
...
test('validates GEMINI_API_KEY via /api/local-validate-secret', async () => {
  const localApi = await setupApiDir({});
  ...
  try {
    ...
    assert.equal(response.json?.message, 'Gemini key verified');
  } finally {
    restoreHttps();
    await app.close();
    await localApi.cleanup();
