...
const OFFICIAL_CHOKEPOINT_SOURCES: Record<string, string> = {
  suez: 'https://www.suezcanal.gov.eg/English/Navigation/Pages/default.aspx',
  panama: 'https://pancanal.com/en/maritime-services/',
  malacca_strait: 'https://www.mpa.gov.sg/',
  hormuz_strait: 'https://www.dpworld.com/en/ports-and-terminals/jebel-ali-port',
  bab_el_mandeb: 'https://www.pma.gov.ye/',
  taiwan_strait: 'https://www.motcmpb.gov.tw/',
  cape_of_good_hope: 'https://www.transnet.net/',
  gibraltar: 'https://www.gibraltarport.com/',
  bosphorus: 'https://www.kiyiemniyeti.gov.tr/',
  korea_strait: 'https://www.busanpa.com/',
  dover_strait: 'https://www.portofdover.com/',
  kerch_strait: 'https://www.fedports.ru/',
  lombok_strait: 'https://www.pelindo.co.id/',
};
...
async function fetchChokepointData(): Promise<ChokepointFetchResult> {
  ...
  // Run TinyFish chokepoint harvesting on schedule (60m lease) without
  // blocking this request path. Responses consume cache-first observations.
  const chokepointPrefetchTargets = CHOKEPOINTS
    .map((cp) => ({
      chokepointId: cp.id,
      chokepointName: cp.name,
      sourceUrl: OFFICIAL_CHOKEPOINT_SOURCES[cp.id] ?? '',
    }))
    .filter((target) => target.sourceUrl.length > 0);
  void prefetchChokepointObservations(chokepointPrefetchTargets).catch(() => {});

  const chokepoints = await Promise.all(CHOKEPOINTS.map(async (cp): Promise<ChokepointInfo> => {
    ...
    const sourceUrl = OFFICIAL_CHOKEPOINT_SOURCES[cp.id] ?? '';
    const tinyfishQueue = sourceUrl
      ? await getChokepointObservationWithTinyfish({
        chokepointId: cp.id,
        chokepointName: cp.name,
        sourceUrl,
      }, { preferLive: false })
      : null;
    const officialQueue = tinyfishQueue?.observation ?? {
      status: 'unknown',
      vesselCount: 0,
      avgWaitHours: 0,
      restrictionSummary: '',
    ...
    return {
      ...
      officialQueue: {
        vesselCount: officialQueue.vesselCount,
        avgWaitHours: officialQueue.avgWaitHours,
        observedAt: officialQueue.observedAt,
        sourceUrl: officialQueue.sourceUrl,
        confidence: officialQueue.confidence,
      },
    };
  }));
