...
const MAX_CRITICAL_DEEP_EXTRACTS = 5;
...
function toProtoItem(item: ParsedItem): ProtoNewsItem {
  return {
    ...
    deepExtract: item.deepExtract ? {
      fullTextAvailable: item.deepExtract.fullTextAvailable,
      infraDamageMentioned: item.deepExtract.infraDamageMentioned,
      coordinates: item.deepExtract.coordinates,
      entities: item.deepExtract.entities,
      extractedAt: item.deepExtract.extractedAt,
      confidence: item.deepExtract.confidence,
    } : undefined,
  };
}

async function enrichCriticalWithTinyfish(items: ParsedItem[]): Promise<void> {
  const criticalItems = items
    .filter((item) => item.level === 'critical' && typeof item.link === 'string' && item.link.length > 0)
    .slice(0, MAX_CRITICAL_DEEP_EXTRACTS);

  await Promise.allSettled(criticalItems.map(async (item) => {
    const { deepExtract } = await getNewsDeepExtractWithTinyfish({
      title: item.title,
      link: item.link,
      source: item.source,
      threatLevel: LEVEL_TO_PROTO[item.level],
    }, { preferLive: false });
    item.deepExtract = deepExtract;
  }));
}
...
async function buildDigest(variant: string, lang: string): Promise<ListFeedDigestResponse> {
  ...
  try {
    ...
    const deepExtractPrefetchItems = allSliced
      .filter((item) => item.level === 'critical' && item.link.length > 0)
      .map((item) => ({
        title: item.title,
        link: item.link,
        source: item.source,
    ...
    void prefetchCriticalNewsDeepExtracts(deepExtractPrefetchItems).catch(() => {});
    await enrichCriticalWithTinyfish(allSliced);
