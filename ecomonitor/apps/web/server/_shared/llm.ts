...
export type LlmProviderName = 'ollama' | 'gemini' | 'groq' | 'openrouter' | 'generic';
...
export function getProviderCredentials(
  ...
  if (provider === 'gemini') {
    ...
    return {
      ...
      model: overrides.model || process.env.GEMINI_MODEL || 'gemini-2.5-flash',
      headers: {
        'Authorization': `Bearer ${apiKey}`,
        'Content-Type': 'application/json',
      },
  ...
  if (provider === 'openrouter') {
    ...
    return {
      ...
      headers: {
        'Authorization': `Bearer ${apiKey}`,
        'Content-Type': 'application/json',
        'HTTP-Referer': 'https://ecomonitor.app',
        'X-Title': 'EcoMonitor',
      },
    };
  }
...
const PROVIDER_CHAIN = ['ollama', 'gemini', 'groq', 'openrouter', 'generic'] as const;
