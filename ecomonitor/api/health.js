...
const TINYFISH_KEYS = {
  providerHealth: 'wm:tinyfish:provider-health',
  lastPrefetch: 'wm:tinyfish:last-prefetch-metrics',
  runtimeSettings: 'wm:tinyfish:settings:v1',
};
...
export default async function handler(req) {
  ...
  const allTinyfishKeys = Object.values(TINYFISH_KEYS);
  const allKeys = [...allDataKeys, ...allMetaKeys, ...allTinyfishKeys];
  ...
  const tinyfishProvider = parseRedisValue(keyValues.get(TINYFISH_KEYS.providerHealth));
  const tinyfishPrefetch = parseRedisValue(keyValues.get(TINYFISH_KEYS.lastPrefetch));
  const tinyfishSettings = parseRedisValue(keyValues.get(TINYFISH_KEYS.runtimeSettings));

  let tinyfishAgeMin = null;
  if (tinyfishPrefetch?.createdAt) {
    const parsed = Date.parse(tinyfishPrefetch.createdAt);
    if (Number.isFinite(parsed)) {
      tinyfishAgeMin = Math.round((now - parsed) / 60_000);
    }
  }
  const tinyfishFreshnessStatus = tinyfishAgeMin == null
    ? 'unknown'
    : tinyfishAgeMin <= 60
      ? 'fresh'
      : 'stale';
  ...
  const body = {
    ...
    checkedAt: new Date(now).toISOString(),
    tinyfish: {
      provider: tinyfishProvider ?? { status: 'unavailable' },
      cacheFreshness: {
        status: tinyfishFreshnessStatus,
        ageMinutes: tinyfishAgeMin,
      },
      lastPrefetch: tinyfishPrefetch ?? null,
      settings: tinyfishSettings ?? null,
    },
  };
